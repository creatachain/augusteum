...
type AppConnTest interface {
	EchoAsync(string) *msmcli.ReqRes
	FlushSync() error
...
func NewAppConnTest(appConn msmcli.Client) AppConnTest {
	return &appConnTest{appConn}
}

func (app *appConnTest) EchoAsync(msg string) *msmcli.ReqRes {
...
func TestEcho(t *testing.T) {
	...
	s.SetLogger(log.TestingLogger().With("module", "msm-server"))
	...
	// Start client
	cli, err := clientCreator.NewMSMClient()
	if err != nil {
		t.Fatalf("Error creating MSM client: %v", err.Error())
	}
	cli.SetLogger(log.TestingLogger().With("module", "msm-client"))
	if err := cli.Start(); err != nil {
		t.Fatalf("Error starting MSM client: %v", err.Error())
	}
...
func BenchmarkEcho(b *testing.B) {
	...
	cli, err := clientCreator.NewMSMClient()
	if err != nil {
		b.Fatalf("Error creating MSM client: %v", err.Error())
	}
	cli.SetLogger(log.TestingLogger().With("module", "msm-client"))
	if err := cli.Start(); err != nil {
		b.Fatalf("Error starting MSM client: %v", err.Error())
	}
