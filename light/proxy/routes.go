...
func RPCRoutes(c *lrpc.Client) map[string]*rpcserver.RPCFunc {
	return map[string]*rpcserver.RPCFunc{
		...
		// msm API
		"msm_query": rpcserver.NewRPCFunc(makeMSMQueryFunc(c), "path,data,height,prove"),
		"msm_info":  rpcserver.NewRPCFunc(makeMSMInfoFunc(c), ""),
...
type rpcMSMQueryFunc func(ctx *rpctypes.Context, path string,
	data bytes.HexBytes, height int64, prove bool) (*ctypes.ResultMSMQuery, error)

func makeMSMQueryFunc(c *lrpc.Client) rpcMSMQueryFunc {
	return func(ctx *rpctypes.Context, path string, data bytes.HexBytes,
		height int64, prove bool) (*ctypes.ResultMSMQuery, error) {

		return c.MSMQueryWithOptions(ctx.Context(), path, data, rpcclient.MSMQueryOptions{
			Height: height,
			Prove:  prove,
		})
	}
...
type rpcMSMInfoFunc func(ctx *rpctypes.Context) (*ctypes.ResultMSMInfo, error)

func makeMSMInfoFunc(c *lrpc.Client) rpcMSMInfoFunc {
	return func(ctx *rpctypes.Context) (*ctypes.ResultMSMInfo, error) {
		return c.MSMInfo(ctx.Context())
	}
}
