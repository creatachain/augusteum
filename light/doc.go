...
Augusteum light clients allow bandwidth & compute-constrained devices, such as
smartphones, low-power embedded chips, or other blockchains to efficiently
verify the consensus of a Augusteum blockchain. This forms the basis of safe
and efficient state synchronization for new network nodes and inter-blockchain
communication (where a light client of one Augusteum instance runs in another
...
NOTE: Augusteum provides a somewhat different (stronger) light client model
...
https://github.com/creatachain/spec/blob/master/spec/consensus/light-client/verification.md
for details.
...
Augusteum RPC exposes a lot of info, but a malicious node could return any
...
See
https://docs.augusteum.com/master/augusteum-core/light-client-protocol.html
for usage example.
Or see
https://github.com/creatachain/spec/tree/master/spec/consensus/light-client
for the full spec
*/
package light
