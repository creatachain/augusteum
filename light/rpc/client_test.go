...
func TestMSMQuery(t *testing.T) {
	...
	next.On(
		...
		bytes.HexBytes(key),
		mock.AnythingOfType("client.MSMQueryOptions"),
	).Return(&ctypes.ResultMSMQuery{
		Response: msm.ResponseQuery{
			Code:   0,
			Key:    key,
			Value:  value,
			Height: 1,
	...
	res, err := c.MSMQuery(context.Background(), "/store/accounts/key", key)
	require.NoError(t, err)
