...
import (
	...
	service "github.com/creatachain/augusteum/libs/service"
...
type Client struct {
	...
	lc   LightClient
	// Proof runtime used to verify values returned by MSMQuery
...
// to call MSMQuery or MSMQueryWithOptions.
...
func (c *Client) MSMInfo(ctx context.Context) (*ctypes.ResultMSMInfo, error) {
	return c.next.MSMInfo(ctx)
}

// MSMQuery requests proof by default.
func (c *Client) MSMQuery(ctx context.Context, path string, data tmbytes.HexBytes) (*ctypes.ResultMSMQuery, error) {
	return c.MSMQueryWithOptions(ctx, path, data, rpcclient.DefaultMSMQueryOptions)
}

// MSMQueryWithOptions returns an error if opts.Prove is false.
func (c *Client) MSMQueryWithOptions(ctx context.Context, path string, data tmbytes.HexBytes,
	opts rpcclient.MSMQueryOptions) (*ctypes.ResultMSMQuery, error) {

	// always request the proof
	opts.Prove = true

	res, err := c.next.MSMQueryWithOptions(ctx, path, data, opts)
	if err != nil {
		return nil, err
	}
	resp := res.Response
	...
	return &ctypes.ResultMSMQuery{Response: resp}, nil
}
...
func (c *Client) BlockResults(ctx context.Context, height *int64) (*ctypes.ResultBlockResults, error) {
	...
	bbeBytes, err := proto.Marshal(&msm.ResponseBeginBlock{
	...
	ebeBytes, err := proto.Marshal(&msm.ResponseEndBlock{
