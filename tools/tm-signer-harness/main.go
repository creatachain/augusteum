...
import (
	...
	"github.com/creatachain/augusteum/tools/tm-signer-harness/internal"
...
func init() {
	...
	rootCmd.Usage = func() {
		fmt.Println(`Remote signer test harness for Augusteum.

Usage:
  tm-signer-harness <command> [flags]

Available Commands:
  extract_key        Extracts a signing key from a local Augusteum instance
...
Use "tm-signer-harness help <command>" for more information about that command.`)
	...
	runCmd.StringVar(&flagTMHome, "tmhome", defaultTMHome, "Path to the Augusteum home directory")
	runCmd.Usage = func() {
		fmt.Println(`Runs the remote signer test harness for Augusteum.
...
Flags:`)
	...
	extractKeyCmd.StringVar(&flagTMHome, "tmhome", defaultTMHome, "Path to the Augusteum home directory")
	extractKeyCmd.Usage = func() {
		fmt.Println(`Extracts a signing key from a local Augusteum instance for use in the remote
signer under test.
...
Flags:`)
	...
	versionCmd.Usage = func() {
		fmt.Println(`
Prints the Augusteum version for which this remote signer harness was built.
