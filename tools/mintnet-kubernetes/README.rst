...
.. figure:: assets/t_plus_k.png
   :alt: Augusteum plus Kubernetes

   Augusteum plus Kubernetes
...
::
    ...
    git clone https://github.com/creatachain/tools.git && cd tools/mintnet-kubernetes/examples/basecoin && make create
...
then query the Augusteum app logs from the first pod:

``kubectl logs -c tm -f tm-0``

finally, use our `Rest API <https://augusteum.com/docs/augusteum-core/rpc.html>`__ to fetch the status of the second pod's Augusteum app.
...
::
    ...
    curl -Lo app.yaml https://github.com/creatachain/tools/raw/master/mintnet-kubernetes/app.template.yaml
...
Augusteum app logs from the first pod:
...
Status of the second pod's Augusteum app:
...
keys, it forms a genesis file and starts the Augusteum process.
