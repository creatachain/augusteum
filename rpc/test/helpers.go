...
func createConfig() *cfg.Config {
	...
	c.RPC.CORSAllowedOrigins = []string{"https://augusteum.com/"}
...
// StartAugusteum starts a test augusteum server in a go routine and returns when it is initialized
func StartAugusteum(app msm.Application, opts ...func(*Options)) *nm.Node {
	nodeOpts := defaultOptions
	for _, opt := range opts {
		opt(&nodeOpts)
	}
	node := NewAugusteum(app, &nodeOpts)
	err := node.Start()
	if err != nil {
		panic(err)
	}
	...
	if !nodeOpts.suppressStdout {
		fmt.Println("Augusteum running!")
	}

	return node
}

// StopAugusteum stops a test augusteum server, waits until it's stopped and
// cleans up test/config files.
func StopAugusteum(node *nm.Node) {
...
// NewAugusteum creates a new augusteum server and sleeps forever
func NewAugusteum(app msm.Application, opts *Options) *nm.Node {
	// Create & start node
...
// SuppressStdout is an option that tries to make sure the RPC test Augusteum
