...
import (
	...
	jsonrpcclient "github.com/creatachain/augusteum/rpc/jsonrpc/client"
	"github.com/creatachain/augusteum/types"
)

/*
HTTP is a Client implementation that communicates with a Augusteum node over
JSON RPC and WebSockets.

This is the main implementation you probably want to use in production code.
There are other implementations when calling the Augusteum node in-process
(Local), or when you want to mock out the server for test code (mock).

You can subscribe for any event published by Augusteum using Subscribe method.
Note delivery is best-effort. If you don't read events fast enough or network is
slow, Augusteum might cancel the subscription. The client will attempt to
...
func (c *baseRPCClient) MSMInfo(ctx context.Context) (*ctypes.ResultMSMInfo, error) {
	result := new(ctypes.ResultMSMInfo)
	_, err := c.caller.Call(ctx, "msm_info", map[string]interface{}{}, result)
	if err != nil {
		return nil, err
	}
...
func (c *baseRPCClient) MSMQuery(
	ctx context.Context,
	path string,
	data bytes.HexBytes,
) (*ctypes.ResultMSMQuery, error) {
	return c.MSMQueryWithOptions(ctx, path, data, rpcclient.DefaultMSMQueryOptions)
}

func (c *baseRPCClient) MSMQueryWithOptions(
	ctx context.Context,
	path string,
	data bytes.HexBytes,
	opts rpcclient.MSMQueryOptions) (*ctypes.ResultMSMQuery, error) {
	result := new(ctypes.ResultMSMQuery)
	_, err := c.caller.Call(ctx, "msm_query",
...
func (w *WSEvents) Subscribe(ctx context.Context, subscriber, query string,
	...
	w.mtx.Lock()
	// subscriber param is ignored because Augusteum will override it with
	// remote IP anyway.
	...
	for {
		...
		case resp, ok := <-w.ws.ResponsesCh:
			...
			if resp.Error != nil {
				...
				// client) reached or Augusteum exited.
				// We can ignore ErrAlreadySubscribed, but need to retry in other
				// cases.
				if !isErrAlreadySubscribed(resp.Error) {
					// Resubscribe after 1 second to give Augusteum time to restart (if
					// crashed).
