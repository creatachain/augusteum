...
import (
	...
	"github.com/creatachain/augusteum/rpc/core"
...
* Communication between an MSM app and Augusteum core when they
...
Note delivery is best-effort. If you don't read events fast enough, Augusteum
...
func (c *Local) MSMInfo(ctx context.Context) (*ctypes.ResultMSMInfo, error) {
	return core.MSMInfo(c.ctx)
}

func (c *Local) MSMQuery(ctx context.Context, path string, data bytes.HexBytes) (*ctypes.ResultMSMQuery, error) {
	return c.MSMQueryWithOptions(ctx, path, data, rpcclient.DefaultMSMQueryOptions)
}

func (c *Local) MSMQueryWithOptions(
	ctx context.Context,
	path string,
	data bytes.HexBytes,
	opts rpcclient.MSMQueryOptions) (*ctypes.ResultMSMQuery, error) {
	return core.MSMQuery(c.ctx, path, data, opts.Height, opts.Prove)
}
