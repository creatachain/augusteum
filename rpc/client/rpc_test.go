...
import (
	...
	rpclocal "github.com/creatachain/augusteum/rpc/client/local"
	ctypes "github.com/creatachain/augusteum/rpc/core/types"
	rpcclient "github.com/creatachain/augusteum/rpc/jsonrpc/client"
...
func TestInfo(t *testing.T) {
	for i, c := range GetClients() {
		// status, err := c.Status()
		// require.Nil(t, err, "%+v", err)
		info, err := c.MSMInfo(context.Background())
		require.Nil(t, err, "%d: %+v", i, err)
...
func TestMSMQuery(t *testing.T) {
	for i, c := range GetClients() {
		...
		require.NoError(t, err)
		res, err := c.MSMQuery(context.Background(), "/key", k)
		qres := res.Response
...
func TestAppCalls(t *testing.T) {
	...
	for i, c := range GetClients() {
		...
		_qres, err := c.MSMQueryWithOptions(context.Background(), "/key", k, client.MSMQueryOptions{Prove: false})
		require.NoError(err)
		qres := _qres.Response
		...
		_pres, err := c.MSMQueryWithOptions(context.Background(), "/key", k, client.MSMQueryOptions{Prove: true})
		require.NoError(err)
		pres := _pres.Response
...
func TestBroadcastTxSync(t *testing.T) {
	...
	for i, c := range GetClients() {
		...
		require.Nil(err, "%d: %+v", i, err)
		require.Equal(bres.Code, msm.CodeTypeOK) // FIXME
...
func TestUnconfirmedTxs(t *testing.T) {
	...
	ch := make(chan *msm.Response, 1)
	mempool := node.Mempool()
	err := mempool.CheckTx(tx, func(resp *msm.Response) { ch <- resp }, mempl.TxInfo{})
	require.NoError(t, err)
...
func TestCheckTx(t *testing.T) {
	...
	for _, c := range GetClients() {
		...
		require.NoError(t, err)
		assert.Equal(t, msm.CodeTypeOK, res.Code)
...
func TestTxSearchWithTimeout(t *testing.T) {
	...
	result, err := timeoutClient.TxSearch(context.Background(), "app.creator='Creatahi Netowoko'", false, nil, nil, "asc")
	require.Nil(t, err)
...
func TestTxSearch(t *testing.T) {
	...
	for i, c := range GetClients() {
		...
		// query using a compositeKey (see kvstore application)
		result, err = c.TxSearch(context.Background(), "app.creator='Creatahi Netowoko'", false, nil, nil, "asc")
		require.Nil(t, err)
		require.Greater(t, len(result.Txs), 0, "expected a lot of transactions")
		...
		result, err = c.TxSearch(context.Background(),
			"app.creator='Creatahi Netowoko' AND tx.height<10000", true, nil, nil, "asc")
		require.Nil(t, err)
		require.Greater(t, len(result.Txs), 0, "expected a lot of transactions")

		// query a non existing tx with page 1 and txsPerPage 1
		perPage := 1
		result, err = c.TxSearch(context.Background(), "app.creator='Creatahi Neetowoko'", true, nil, &perPage, "asc")
		require.Nil(t, err)
		require.Len(t, result.Txs, 0)
...
func testBatchedJSONRPCCalls(t *testing.T, c *rpchttp.HTTP) {
	...
	q1, err := batch.MSMQuery(context.Background(), "/key", k1)
	require.NoError(t, err)
	q2, err := batch.MSMQuery(context.Background(), "/key", k2)
	require.NoError(t, err)
	require.Equal(t, 2, batch.Count())
	...
	qresult1, ok := qresults[0].(*ctypes.ResultMSMQuery)
	require.True(t, ok)
	require.Equal(t, *qresult1, *q1)
	qresult2, ok := qresults[1].(*ctypes.ResultMSMQuery)
	require.True(t, ok)
