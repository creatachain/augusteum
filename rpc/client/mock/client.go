...
want to directly call a augusteum node in process, you can use the
"local" package.
*/
...
func (c Client) MSMInfo(ctx context.Context) (*ctypes.ResultMSMInfo, error) {
	return core.MSMInfo(&rpctypes.Context{})
}

func (c Client) MSMQuery(ctx context.Context, path string, data bytes.HexBytes) (*ctypes.ResultMSMQuery, error) {
	return c.MSMQueryWithOptions(ctx, path, data, client.DefaultMSMQueryOptions)
}

func (c Client) MSMQueryWithOptions(
	ctx context.Context,
	path string,
	data bytes.HexBytes,
	opts client.MSMQueryOptions) (*ctypes.ResultMSMQuery, error) {
	return core.MSMQuery(&rpctypes.Context{}, path, data, opts.Height, opts.Prove)
}
