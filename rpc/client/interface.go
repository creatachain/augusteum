...
to a augusteum node, as well as higher-level functionality.

The main implementation for production code is client.HTTP, which
connects via http to the jsonrpc interface of the augusteum node.

For connecting to a node running in the same process (eg. when
compiling the msm app in the same process), you can use the client.Local
implementation.
...
// MSMClient groups together the functionality that principally affects the
// MSM app.
//
// In many cases this will be all we want, so we can accept an interface which
// is easier to mock.
type MSMClient interface {
	// Reading from msm app
	MSMInfo(context.Context) (*ctypes.ResultMSMInfo, error)
	MSMQuery(ctx context.Context, path string, data bytes.HexBytes) (*ctypes.ResultMSMQuery, error)
	MSMQueryWithOptions(ctx context.Context, path string, data bytes.HexBytes,
		opts MSMQueryOptions) (*ctypes.ResultMSMQuery, error)
...
// string. see augusteum/types/events.go
