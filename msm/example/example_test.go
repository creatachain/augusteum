...
func testStream(t *testing.T, app types.Application) {
	...
	client.SetLogger(log.TestingLogger().With("module", "msm-client"))
...
func testGRPCSync(t *testing.T, app types.MSMApplicationServer) {
	numDeliverTxs := 2000
	socketFile := fmt.Sprintf("test-%08x.sock", rand.Int31n(1<<30))
	defer os.Remove(socketFile)
	socket := fmt.Sprintf("unix://%v", socketFile)
	...
	// Start the listener
	server := msmserver.NewGRPCServer(socket, app)
	server.SetLogger(log.TestingLogger().With("module", "msm-server"))
	if err := server.Start(); err != nil {
	...
	client := types.NewMSMApplicationClient(conn)
