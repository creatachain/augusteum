...
func TestPersistentKVStoreKV(t *testing.T) {
	dir, err := ioutil.TempDir("/tmp", "msm-kvstore-test") // TODO
	if err != nil {
		t.Fatal(err)
	}
...
func makeSocketClientServer(app types.Application, name string) (msmcli.Client, service.Service, error) {
	// Start the listener
	socket := fmt.Sprintf("unix://%s.sock", name)
	logger := log.TestingLogger()

	server := msmserver.NewSocketServer(socket, app)
	server.SetLogger(logger.With("module", "msm-server"))
	if err := server.Start(); err != nil {
		return nil, nil, err
	}

	// Connect to the socket
	client := msmcli.NewSocketClient(socket, false)
...
func makeGRPCClientServer(app types.Application, name string) (msmcli.Client, service.Service, error) {
	// Start the listener
	socket := fmt.Sprintf("unix://%s.sock", name)
	logger := log.TestingLogger()

	gapp := types.NewGRPCApplication(app)
	server := msmserver.NewGRPCServer(socket, gapp)
	server.SetLogger(logger.With("module", "msm-server"))
	if err := server.Start(); err != nil {
		return nil, nil, err
	}

	client := msmcli.NewGRPCClient(socket, true)
	client.SetLogger(logger.With("module", "msm-client"))
	if err := client.Start(); err != nil {
		if err := server.Stop(); err != nil {
			return nil, nil, err
...
func runClientTests(t *testing.T, client msmcli.Client) {
	// run some tests....
	key := testKey
	value := key
	tx := []byte(key)
...
func testClient(t *testing.T, app msmcli.Client, tx []byte, key, value string) {
