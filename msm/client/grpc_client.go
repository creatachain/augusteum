...
type grpcClient struct {
	...
	client   types.MSMApplicationClient
...
RETRY_LOOP:
	for {
		...
		if err != nil {
			...
			cli.Logger.Error(fmt.Sprintf("msm.grpcClient failed to connect to %v.  Retrying...\n", cli.addr), "err", err)
...
func (cli *grpcClient) StopForError(err error) {
	...
	cli.Logger.Error(fmt.Sprintf("Stopping msm.grpcClient for error: %v", err.Error()))
	if err := cli.Stop(); err != nil {
		cli.Logger.Error("Error stopping msm.grpcClient", "err", err)
	}
}
