...
var killCmd = &cobra.Command{
	...
	Short: "Kill a Augusteum process while aggregating and packaging debugging data",
	Long: `Kill a Augusteum process while also aggregating Augusteum process data
...
Example:
$ augusteum debug 34255 /path/to/tm-debug.zip`,
...
func killCmdHandler(cmd *cobra.Command, args []string) error {
	...
	tmpDir, err := ioutil.TempDir(os.TempDir(), "augusteum_debug_tmp")
	if err != nil {
	...
	logger.Info("killing Augusteum process")
...
// killProc attempts to kill the Augusteum process with a given PID with an
...
func killProc(pid uint64, dir string) error {
	// pipe STDERR output from tailing the Augusteum process to a file
	//
	...
	// kill the underlying Augusteum process and subsequent tailing process
	go func() {
		// Killing the Augusteum process with the '-ABRT|-6' signal will result in
		// a goroutine stacktrace.
		p, err := os.FindProcess(int(pid))
		if err != nil {
			fmt.Fprintf(os.Stderr, "failed to find PID to kill Augusteum process: %s", err)
		} else if err = p.Signal(syscall.SIGABRT); err != nil {
			fmt.Fprintf(os.Stderr, "failed to kill Augusteum process: %s", err)
		}

		// allow some time to allow the Augusteum process to be killed
		//
		...
		if err := cmd.Process.Kill(); err != nil {
			fmt.Fprintf(os.Stderr, "failed to kill Augusteum process output redirection: %s", err)
		}
	}()
