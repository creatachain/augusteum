...
// dumpStatus gets node status state dump from the Augusteum RPC and writes it
...
// dumpNetInfo gets network information state dump from the Augusteum RPC and
...
// dumpConsensusState gets consensus state dump from the Augusteum RPC and
// writes it to file. It returns an error upon failure.
...
// copyWAL copies the Augusteum node's WAL file. It returns an error if the
...
// copyConfig copies the Augusteum node's config file. It returns an error if
