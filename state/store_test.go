	...
	for name, tc := range testcases {
		...
		t.Run(name, func(t *testing.T) {
			...
			for h := int64(1); h <= tc.makeHeights; h++ {
				...
				err = stateStore.SaveMSMResponses(h, &tmstate.MSMResponses{
					DeliverTxs: []*msm.ResponseDeliverTx{
						{Data: []byte{1}},
						{Data: []byte{2}},
						{Data: []byte{3}},
					},
			...
			expectMSM := sliceToMap(tc.expectMSM)
			...
			for h := int64(1); h <= tc.makeHeights; h++ {
				...
				msm, err := stateStore.LoadMSMResponses(h)
				if expectMSM[h] {
					require.NoError(t, err, "msm height %v", h)
					require.NotNil(t, msm)
				} else {
					require.Error(t, err, "msm height %v", h)
					require.Equal(t, sm.ErrNoMSMResponsesForHeight{Height: h}, err)
				}
			}
		})
	}
...
func TestMSMResponsesResultsHash(t *testing.T) {
	...
	root := sm.MSMResponsesResultsHash(responses)
