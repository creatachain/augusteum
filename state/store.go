...
const (
	...
	// https://github.com/creatachain/augusteum/pull/3438
...
func calcMSMResponsesKey(height int64) []byte {
	return []byte(fmt.Sprintf("msmResponsesKey:%v", height))
}
...
//
// It is used to retrieve current state and save and load MSM responses,
...
type Store interface {
	...
	// LoadMSMResponses loads the msmResponse for a given height
	LoadMSMResponses(int64) (*tmstate.MSMResponses, error)
	// LoadConsensusParams loads the consensus params for a given height
	LoadConsensusParams(int64) (tmproto.ConsensusParams, error)
	// Save overwrites the previous state with the updated one
	Save(State) error
	// SaveMSMResponses saves MSMResponses for a given height
	SaveMSMResponses(int64, *tmstate.MSMResponses) error
...
// dbStore wraps a db (github.com/creatachain/tm-db)
type dbStore struct {
	db dbm.DB
}
...
func (store dbStore) save(state State, key []byte) error {
	...
	if nextHeight == 1 {
		...
		// This extra logic due to Augusteum validator set changes being delayed 1 block.
...
// encoding not preserving ordering: https://github.com/creatachain/augusteum/issues/4567
...
func (store dbStore) PruneStates(from int64, to int64) error {
	...
	for h := to - 1; h >= from; h-- {
		...
		err = batch.Delete(calcMSMResponsesKey(h))
		if err != nil {
			return err
		}
		pruned++
...
// MSMResponsesResultsHash returns the root hash of a Merkle tree of
// ResponseDeliverTx responses (see MSMResults.Hash)
//
// See merkle.SimpleHashFromByteSlices
func MSMResponsesResultsHash(ar *tmstate.MSMResponses) []byte {
	return types.NewResults(ar.DeliverTxs).Hash()
}

// LoadMSMResponses loads the MSMResponses for the given height from the
// database. If not found, ErrNoMSMResponsesForHeight is returned.
//
...
// the result of txs.
func (store dbStore) LoadMSMResponses(height int64) (*tmstate.MSMResponses, error) {
	buf, err := store.db.Get(calcMSMResponsesKey(height))
	if err != nil {
		return nil, err
	}
	if len(buf) == 0 {

		return nil, ErrNoMSMResponsesForHeight{height}
	}

	msmResponses := new(tmstate.MSMResponses)
	err = msmResponses.Unmarshal(buf)
	if err != nil {
		// DATA HAS BEEN CORRUPTED OR THE SPEC HAS CHANGED
		tmos.Exit(fmt.Sprintf(`LoadMSMResponses: Data has been corrupted or its spec has
                changed: %v\n`, err))
	}
...
// SaveMSMResponses persists the MSMResponses to the database.
// This is useful in case we crash after app.Commit and before s.Save().
// Responses are indexed by height so they can also be loaded later to produce
// Merkle proofs.
//
// Exposed for testing.
func (store dbStore) SaveMSMResponses(height int64, msmResponses *tmstate.MSMResponses) error {
	var dtxs []*msm.ResponseDeliverTx
	// strip nil values,
	for _, tx := range msmResponses.DeliverTxs {
		if tx != nil {
			dtxs = append(dtxs, tx)
		}
	}
	...
	bz, err := msmResponses.Marshal()
	if err != nil {
		return err
	}

	err = store.db.SetSync(calcMSMResponsesKey(height), bz)
	if err != nil {
		return err
	}
