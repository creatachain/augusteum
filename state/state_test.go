...
// TestMSMResponsesSaveLoad tests saving and loading MSMResponses.
func TestMSMResponsesSaveLoad1(t *testing.T) {
	tearDown, stateDB, state := setupTestCase(t)
	defer tearDown(t)
	stateStore := sm.NewStore(stateDB)
	assert := assert.New(t)
	...
	msmResponses := new(tmstate.MSMResponses)
	dtxs := make([]*msm.ResponseDeliverTx, 2)
	msmResponses.DeliverTxs = dtxs

	msmResponses.DeliverTxs[0] = &msm.ResponseDeliverTx{Data: []byte("foo"), Events: nil}
	msmResponses.DeliverTxs[1] = &msm.ResponseDeliverTx{Data: []byte("bar"), Log: "ok", Events: nil}
	msmResponses.EndBlock = &msm.ResponseEndBlock{ValidatorUpdates: []msm.ValidatorUpdate{
		types.TM2PB.NewValidatorUpdate(ed25519.GenPrivKey().PubKey(), 10),
	}}

	err := stateStore.SaveMSMResponses(block.Height, msmResponses)
	require.NoError(t, err)
	loadedMSMResponses, err := stateStore.LoadMSMResponses(block.Height)
	assert.Nil(err)
	assert.Equal(msmResponses, loadedMSMResponses,
		fmt.Sprintf("MSMResponses don't match:\ngot:       %v\nexpected: %v\n",
			loadedMSMResponses, msmResponses))
}

// TestResultsSaveLoad tests saving and loading MSM results.
func TestMSMResponsesSaveLoad2(t *testing.T) {
	...
	cases := [...]struct {
		...
		added    []*msm.ResponseDeliverTx
		expected []*msm.ResponseDeliverTx
	}{
		0: {
			nil,
			nil,
		...
		2: {
			[]*msm.ResponseDeliverTx{
				{Code: 383},
				{
					Data: []byte("Gotcha!"),
					Events: []msm.Event{
						{Type: "type1", Attributes: []msm.EventAttribute{{Key: []byte("a"), Value: []byte("1")}}},
						{Type: "type2", Attributes: []msm.EventAttribute{{Key: []byte("build"), Value: []byte("stuff")}}},
					},
				},
			},
			[]*msm.ResponseDeliverTx{
				{Code: 383, Data: nil},
				{Code: 0, Data: []byte("Gotcha!"), Events: []msm.Event{
					{Type: "type1", Attributes: []msm.EventAttribute{{Key: []byte("a"), Value: []byte("1")}}},
					{Type: "type2", Attributes: []msm.EventAttribute{{Key: []byte("build"), Value: []byte("stuff")}}},
				}},
			}},
	...
	for i := range cases {
		h := int64(i + 1)
		res, err := stateStore.LoadMSMResponses(h)
	...
	for i, tc := range cases {
		...
		responses := &tmstate.MSMResponses{
			BeginBlock: &msm.ResponseBeginBlock{},
			DeliverTxs: tc.added,
			EndBlock:   &msm.ResponseEndBlock{},
		}
		err := stateStore.SaveMSMResponses(h, responses)
		require.NoError(t, err)
	}
	...
	for i, tc := range cases {
		...
		if assert.NoError(err, "%d", i) {
			...
			assert.Equal(sm.MSMResponsesResultsHash(responses), sm.MSMResponsesResultsHash(res), "%d", i)
		}
	}
}
...
// see https://github.com/creatachain/augusteum/issues/2718
func TestProposerPriorityDoesNotGetResetToZero(t *testing.T) {
	...
	validatorUpdates, err := types.PB2TM.ValidatorUpdates(msmResponses.EndBlock.ValidatorUpdates)
	require.NoError(t, err)
	updatedState, err := sm.UpdateState(state, blockID, &block.Header, msmResponses, validatorUpdates)
	...
	updateAddVal := msm.ValidatorUpdate{PubKey: fvp, Power: val2VotingPower}
	validatorUpdates, err = types.PB2TM.ValidatorUpdates([]msm.ValidatorUpdate{updateAddVal})
	assert.NoError(t, err)
	updatedState2, err := sm.UpdateState(updatedState, blockID, &block.Header, msmResponses, validatorUpdates)
	assert.NoError(t, err)
	...
	updateVal := msm.ValidatorUpdate{PubKey: fvp, Power: updatedVotingPowVal2}
	validatorUpdates, err = types.PB2TM.ValidatorUpdates([]msm.ValidatorUpdate{updateVal})
	assert.NoError(t, err)

	// this will cause the diff of priorities (77)
	// to be larger than threshold == 2*totalVotingPower (22):
	updatedState3, err := sm.UpdateState(updatedState2, blockID, &block.Header, msmResponses, validatorUpdates)
	assert.NoError(t, err)
...
func TestProposerPriorityProposerAlternates(t *testing.T) {
	...
	fvp, err := cryptoenc.PubKeyToProto(val2PubKey)
	require.NoError(t, err)
	updateAddVal := msm.ValidatorUpdate{PubKey: fvp, Power: val1VotingPower}
	validatorUpdates, err = types.PB2TM.ValidatorUpdates([]msm.ValidatorUpdate{updateAddVal})
	assert.NoError(t, err)
	...
	oldState, err = sm.UpdateState(oldState, blockID, &block.Header, msmResponses, validatorUpdates)
	assert.NoError(t, err)
	expectedVal1Prio2 = 1
	expectedVal2Prio2 = -1
	expectedVal1Prio = -9
	...
	for i := 0; i < 1000; i++ {
		...
		updatedState, err := sm.UpdateState(oldState, blockID, &block.Header, msmResponses, validatorUpdates)
		assert.NoError(t, err)
...
func TestLargeGenesisValidator(t *testing.T) {
	...
	// see: https://github.com/creatachain/augusteum/issues/2960
	...
	require.NoError(t, err)
	firstAddedVal := msm.ValidatorUpdate{PubKey: fvp, Power: firstAddedValVotingPower}
	validatorUpdates, err := types.PB2TM.ValidatorUpdates([]msm.ValidatorUpdate{firstAddedVal})
	assert.NoError(t, err)
	msmResponses := &tmstate.MSMResponses{
		BeginBlock: &msm.ResponseBeginBlock{},
		EndBlock:   &msm.ResponseEndBlock{ValidatorUpdates: []msm.ValidatorUpdate{firstAddedVal}},
	}
	block := makeBlock(oldState, oldState.LastBlockHeight+1)
	blockID := types.BlockID{Hash: block.Hash(), PartSetHeader: block.MakePartSet(testPartSize).Header()}
	updatedState, err := sm.UpdateState(oldState, blockID, &block.Header, msmResponses, validatorUpdates)
	...
	for i := 0; i < 200; i++ {
		...
		updatedStateInner, err := sm.UpdateState(lastState, blockID, &block.Header, msmResponses, validatorUpdates)
		require.NoError(t, err)
	...
	for i := 0; i < 10; i++ {
		...
		require.NoError(t, err)
		addedVal := msm.ValidatorUpdate{PubKey: ap, Power: firstAddedValVotingPower}
		validatorUpdates, err := types.PB2TM.ValidatorUpdates([]msm.ValidatorUpdate{addedVal})
		assert.NoError(t, err)

		msmResponses := &tmstate.MSMResponses{
			BeginBlock: &msm.ResponseBeginBlock{},
			EndBlock:   &msm.ResponseEndBlock{ValidatorUpdates: []msm.ValidatorUpdate{addedVal}},
		}
		block := makeBlock(oldState, oldState.LastBlockHeight+1)
		blockID := types.BlockID{Hash: block.Hash(), PartSetHeader: block.MakePartSet(testPartSize).Header()}
		state, err = sm.UpdateState(state, blockID, &block.Header, msmResponses, validatorUpdates)
		require.NoError(t, err)
	}
	...
	require.NoError(t, err)
	removeGenesisVal := msm.ValidatorUpdate{PubKey: gp, Power: 0}
	msmResponses = &tmstate.MSMResponses{
		BeginBlock: &msm.ResponseBeginBlock{},
		EndBlock:   &msm.ResponseEndBlock{ValidatorUpdates: []msm.ValidatorUpdate{removeGenesisVal}},
	}
	block = makeBlock(oldState, oldState.LastBlockHeight+1)
	blockID = types.BlockID{Hash: block.Hash(), PartSetHeader: block.MakePartSet(testPartSize).Header()}
	validatorUpdates, err = types.PB2TM.ValidatorUpdates(msmResponses.EndBlock.ValidatorUpdates)
	...
	for isProposerUnchanged {
		...
		blockID = types.BlockID{Hash: block.Hash(), PartSetHeader: block.MakePartSet(testPartSize).Header()}
		curState, err = sm.UpdateState(curState, blockID, &block.Header, msmResponses, validatorUpdates)
		require.NoError(t, err)
	...
	for i := 0; i < 100; i++ {
		...
		updatedState, err = sm.UpdateState(updatedState, blockID, &block.Header, msmResponses, validatorUpdates)
		require.NoError(t, err)
