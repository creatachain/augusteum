...
type TxIndexer interface {
	...
	Index(result *msm.TxResult) error

	// Get returns the transaction specified by hash or nil if the transaction is not indexed
	// or stored.
	Get(hash []byte) (*msm.TxResult, error)

	// Search allows you to query for transactions.
	Search(ctx context.Context, q *query.Query) ([]*msm.TxResult, error)
}
...
func NewBatch(n int64) *Batch {
	return &Batch{
		Ops: make([]*msm.TxResult, n),
	}
}

// Add or update an entry for the given result.Index.
func (b *Batch) Add(result *msm.TxResult) error {
