...
func (txi *TxIndex) indexEvents(result *msm.TxResult, hash []byte, store dbm.Batch) error {
...
func (txi *TxIndex) Search(ctx context.Context, q *query.Query) ([]*msm.TxResult, error) {
	...
	if err != nil {
		...
		switch {
		case err != nil:
			return []*msm.TxResult{}, fmt.Errorf("error while retrieving the result: %w", err)
		case res == nil:
			return []*msm.TxResult{}, nil
		default:
			return []*msm.TxResult{res}, nil
		}
	}
	...
	results := make([]*msm.TxResult, 0, len(filteredHashes))
...
LOOP:
	for ; it.Valid(); it.Next() {
		...
		if _, ok := r.AnyBound().(int64); ok {
			...
			// XXX: passing time in a MSM Events is not yet implemented
			// case time.Time:
...
func keyForEvent(key string, value []byte, result *msm.TxResult) []byte {
...
func keyForHeight(result *msm.TxResult) []byte {
