...
func TestTxIndex(t *testing.T) {
	...
	txResult := &msm.TxResult{
		...
		Result: msm.ResponseDeliverTx{
			Data: []byte{0},
			Code: msm.CodeTypeOK, Log: "", Events: nil,
		},
	}
	hash := tx.Hash()
...
func TestTxSearch(t *testing.T) {
	...
	txResult := txResultWithEvents([]msm.Event{
		{Type: "account", Attributes: []msm.EventAttribute{{Key: []byte("number"), Value: []byte("1"), Index: true}}},
		{Type: "account", Attributes: []msm.EventAttribute{{Key: []byte("owner"), Value: []byte("Ivan"), Index: true}}},
		{Type: "", Attributes: []msm.EventAttribute{{Key: []byte("not_allowed"), Value: []byte("Vlad"), Index: true}}},
	})
...
func TestTxSearchDeprecatedIndexing(t *testing.T) {
	...
	txResult1 := txResultWithEvents([]msm.Event{
		{Type: "account", Attributes: []msm.EventAttribute{{Key: []byte("number"), Value: []byte("1"), Index: true}}},
	})
	...
	testCases := []struct {
		q       string
		results []*msm.TxResult
	}{
		// search by hash
		{fmt.Sprintf("tx.hash = '%X'", hash1), []*msm.TxResult{txResult1}},
		// search by hash
		{fmt.Sprintf("tx.hash = '%X'", hash2), []*msm.TxResult{txResult2}},
		// search by exact match (one key)
		{"account.number = 1", []*msm.TxResult{txResult1}},
		{"account.number >= 1 AND account.number <= 5", []*msm.TxResult{txResult1}},
		// search by range (lower bound)
		{"account.number >= 1", []*msm.TxResult{txResult1}},
		// search by range (upper bound)
		{"account.number <= 5", []*msm.TxResult{txResult1}},
		// search using not allowed key
		{"not_allowed = 'boom'", []*msm.TxResult{}},
		// search for not existing tx result
		{"account.number >= 2 AND account.number <= 5", []*msm.TxResult{}},
		// search using not existing key
		{"account.date >= TIME 2013-05-03T14:45:00Z", []*msm.TxResult{}},
		// search by deprecated key
		{"sender = 'addr1'", []*msm.TxResult{txResult2}},
	}
...
func TestTxSearchOneTxWithMultipleSameTagsButDifferentValues(t *testing.T) {
	...
	txResult := txResultWithEvents([]msm.Event{
		{Type: "account", Attributes: []msm.EventAttribute{{Key: []byte("number"), Value: []byte("1"), Index: true}}},
		{Type: "account", Attributes: []msm.EventAttribute{{Key: []byte("number"), Value: []byte("2"), Index: true}}},
	})
...
func TestTxSearchMultipleTxs(t *testing.T) {
	...
	txResult2 := txResultWithEvents([]msm.Event{
		{Type: "account", Attributes: []msm.EventAttribute{{Key: []byte("number"), Value: []byte("2"), Index: true}}},
	})
	...
	txResult3 := txResultWithEvents([]msm.Event{
		{Type: "account", Attributes: []msm.EventAttribute{{Key: []byte("number"), Value: []byte("3"), Index: true}}},
	})
	...
	// https://github.com/creatachain/augusteum/issues/2908
	txResult4 := txResultWithEvents([]msm.Event{
		{Type: "account", Attributes: []msm.EventAttribute{{Key: []byte("number.id"), Value: []byte("1"), Index: true}}},
	})
...
func txResultWithEvents(events []msm.Event) *msm.TxResult {
	tx := types.Tx("HELLO WORLD")
	return &msm.TxResult{
		Height: 1,
		Index:  0,
