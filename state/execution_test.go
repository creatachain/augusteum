...
import (
	...
	mmock "github.com/creatachain/augusteum/mempool/mock"
	...
	"github.com/creatachain/augusteum/state/mocks"
...
func TestBeginBlockByzantineValidators(t *testing.T) {
	...
	msmEv := []msm.Evidence{
		...
		{
			Type:             msm.EvidenceType_LIGHT_CLIENT_ATTACK,
			Height:           8,
			Time:             defaultEvidenceTime,
			Validator:        types.TM2PB.Validator(state.Validators.Validators[0]),
			TotalVotingPower: 12,
	...
	// TODO check state and mempool
	assert.Equal(t, msmEv, app.ByzantineValidators)
}
...
func TestValidateValidatorUpdates(t *testing.T) {
	...
	defaultValidatorParams := tmproto.ValidatorParams{PubKeyTypes: []string{types.MSMPubKeyTypeEd25519}}

	testCases := []struct {
		name string

		msmUpdates      []msm.ValidatorUpdate
	...
	}{
		{
			...
			[]msm.ValidatorUpdate{{PubKey: pk2, Power: 20}},
			defaultValidatorParams,
			false,
		},
		{
			"updating a validator is OK",
			[]msm.ValidatorUpdate{{PubKey: pk1, Power: 20}},
			defaultValidatorParams,
			false,
		},
		{
			"removing a validator is OK",
			[]msm.ValidatorUpdate{{PubKey: pk2, Power: 0}},
			defaultValidatorParams,
			false,
		},
		{
			"adding a validator with negative power results in error",
			[]msm.ValidatorUpdate{{PubKey: pk2, Power: -100}},
			defaultValidatorParams,
			true,
		},
	}
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			err := sm.ValidateValidatorUpdates(tc.msmUpdates, tc.validatorParams)
			if tc.shouldErr {
				assert.Error(t, err)
			} else {
				assert.NoError(t, err)
...
func TestUpdateValidators(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			"updating a validator is OK",
			types.NewValidatorSet([]*types.Validator{val1}),
			[]msm.ValidatorUpdate{{PubKey: pk, Power: 20}},
	...
	for _, tc := range testCases {
		tc := tc
		t.Run(tc.name, func(t *testing.T) {
			updates, err := types.PB2TM.ValidatorUpdates(tc.msmUpdates)
			assert.NoError(t, err)
...
func TestEndBlockValidatorUpdates(t *testing.T) {
	...
	require.NoError(t, err)
	app.ValidatorUpdates = []msm.ValidatorUpdate{
		{PubKey: pk, Power: 10},
	}
