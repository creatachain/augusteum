...
// syncer runs a state sync against an MSM app. Use either SyncAny() to automatically attempt to
...
func (s *syncer) offerSnapshot(snapshot *snapshot) error {
	s.logger.Info("Offering snapshot to MSM app", "height", snapshot.Height,
		"format", snapshot.Format, "hash", snapshot.Hash)
	resp, err := s.conn.OfferSnapshotSync(msm.RequestOfferSnapshot{
	...
	switch resp.Result {
	case msm.ResponseOfferSnapshot_ACCEPT:
		s.logger.Info("Snapshot accepted, restoring", "height", snapshot.Height,
			"format", snapshot.Format, "hash", snapshot.Hash)
		return nil
	case msm.ResponseOfferSnapshot_ABORT:
		return errAbort
	case msm.ResponseOfferSnapshot_REJECT:
		return errRejectSnapshot
	case msm.ResponseOfferSnapshot_REJECT_FORMAT:
		return errRejectFormat
	case msm.ResponseOfferSnapshot_REJECT_SENDER:
...
func (s *syncer) applyChunks(chunks *chunkQueue) error {
	for {
		...
		resp, err := s.conn.ApplySnapshotChunkSync(msm.RequestApplySnapshotChunk{
			Index:  chunk.Index,
			Chunk:  chunk.Chunk,
		...
		s.logger.Info("Applied snapshot chunk to MSM app", "height", chunk.Height,
		...
		switch resp.Result {
		case msm.ResponseApplySnapshotChunk_ACCEPT:
		case msm.ResponseApplySnapshotChunk_ABORT:
			return errAbort
		case msm.ResponseApplySnapshotChunk_RETRY:
			chunks.Retry(chunk.Index)
		case msm.ResponseApplySnapshotChunk_RETRY_SNAPSHOT:
			return errRetrySnapshot
		case msm.ResponseApplySnapshotChunk_REJECT_SNAPSHOT:
			return errRejectSnapshot
		default:
...
func (s *syncer) verifyApp(snapshot *snapshot) (uint64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("failed to query MSM app for appHash: %w", err)
	}
	...
	if uint64(resp.LastBlockHeight) != snapshot.Height {
		s.logger.Error("MSM app reported unexpected last block height",
			"expected", snapshot.Height, "actual", resp.LastBlockHeight)
		return 0, errVerifyFailed
	}

	s.logger.Info("Verified MSM app", "height", snapshot.Height, "appHash", snapshot.trustedAppHash)
