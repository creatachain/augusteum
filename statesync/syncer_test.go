...
import (
	...
	p2pmocks "github.com/creatachain/augusteum/p2p/mocks"
	tmstate "github.com/creatachain/augusteum/proto/augusteum/state"
	ssproto "github.com/creatachain/augusteum/proto/augusteum/statesync"
	tmversion "github.com/creatachain/augusteum/proto/augusteum/version"
	"github.com/creatachain/augusteum/proxy"
	proxymocks "github.com/creatachain/augusteum/proxy/mocks"
	sm "github.com/creatachain/augusteum/state"
	"github.com/creatachain/augusteum/statesync/mocks"
...
func TestSyncer_SyncAny(t *testing.T) {
	...
	// format 2, which will be rejected by the MSM application.
	...
	connSnapshot.On("OfferSnapshotSync", msm.RequestOfferSnapshot{
	...
	}).Return(&msm.ResponseOfferSnapshot{Result: msm.ResponseOfferSnapshot_REJECT_FORMAT}, nil)
	connSnapshot.On("OfferSnapshotSync", msm.RequestOfferSnapshot{
		Snapshot: &msm.Snapshot{
			Height:   s.Height,
			Format:   s.Format,
		...
		AppHash: []byte("app_hash"),
	}).Times(2).Return(&msm.ResponseOfferSnapshot{Result: msm.ResponseOfferSnapshot_ACCEPT}, nil)
	...
	connSnapshot.On("ApplySnapshotChunkSync", msm.RequestApplySnapshotChunk{
		Index: 2, Chunk: []byte{1, 1, 2},
	}).Once().Run(func(args mock.Arguments) { time.Sleep(2 * time.Second) }).Return(
		&msm.ResponseApplySnapshotChunk{
			Result:        msm.ResponseApplySnapshotChunk_RETRY_SNAPSHOT,
			RefetchChunks: []uint32{1},
		}, nil)

	connSnapshot.On("ApplySnapshotChunkSync", msm.RequestApplySnapshotChunk{
		Index: 0, Chunk: []byte{1, 1, 0},
	}).Times(2).Return(&msm.ResponseApplySnapshotChunk{Result: msm.ResponseApplySnapshotChunk_ACCEPT}, nil)
	connSnapshot.On("ApplySnapshotChunkSync", msm.RequestApplySnapshotChunk{
		Index: 1, Chunk: []byte{1, 1, 1},
	}).Times(2).Return(&msm.ResponseApplySnapshotChunk{Result: msm.ResponseApplySnapshotChunk_ACCEPT}, nil)
	connSnapshot.On("ApplySnapshotChunkSync", msm.RequestApplySnapshotChunk{
		Index: 2, Chunk: []byte{1, 1, 2},
	}).Once().Return(&msm.ResponseApplySnapshotChunk{Result: msm.ResponseApplySnapshotChunk_ACCEPT}, nil)
	connQuery.On("InfoSync", proxy.RequestInfo).Return(&msm.ResponseInfo{
		AppVersion:       9,
		LastBlockHeight:  1,
	...
	// The syncer should have updated the state app version from the MSM info response.
	expectState := state
...
func TestSyncer_SyncAny_abort(t *testing.T) {
	...
	require.NoError(t, err)
	connSnapshot.On("OfferSnapshotSync", msm.RequestOfferSnapshot{
		Snapshot: toMSM(s), AppHash: []byte("app_hash"),
	}).Once().Return(&msm.ResponseOfferSnapshot{Result: msm.ResponseOfferSnapshot_ABORT}, nil)
...
func TestSyncer_SyncAny_reject(t *testing.T) {
	...
	connSnapshot.On("OfferSnapshotSync", msm.RequestOfferSnapshot{
		Snapshot: toMSM(s22), AppHash: []byte("app_hash"),
	}).Once().Return(&msm.ResponseOfferSnapshot{Result: msm.ResponseOfferSnapshot_REJECT}, nil)

	connSnapshot.On("OfferSnapshotSync", msm.RequestOfferSnapshot{
		Snapshot: toMSM(s12), AppHash: []byte("app_hash"),
	}).Once().Return(&msm.ResponseOfferSnapshot{Result: msm.ResponseOfferSnapshot_REJECT}, nil)

	connSnapshot.On("OfferSnapshotSync", msm.RequestOfferSnapshot{
		Snapshot: toMSM(s11), AppHash: []byte("app_hash"),
	}).Once().Return(&msm.ResponseOfferSnapshot{Result: msm.ResponseOfferSnapshot_REJECT}, nil)
...
func TestSyncer_SyncAny_reject_format(t *testing.T) {
	...
	connSnapshot.On("OfferSnapshotSync", msm.RequestOfferSnapshot{
		Snapshot: toMSM(s22), AppHash: []byte("app_hash"),
	}).Once().Return(&msm.ResponseOfferSnapshot{Result: msm.ResponseOfferSnapshot_REJECT_FORMAT}, nil)
...
func TestSyncer_SyncAny_reject_sender(t *testing.T) {
	...
	connSnapshot.On("OfferSnapshotSync", msm.RequestOfferSnapshot{
		Snapshot: toMSM(sbc), AppHash: []byte("app_hash"),
	}).Once().Return(&msm.ResponseOfferSnapshot{Result: msm.ResponseOfferSnapshot_REJECT_SENDER}, nil)

	connSnapshot.On("OfferSnapshotSync", msm.RequestOfferSnapshot{
		Snapshot: toMSM(sa), AppHash: []byte("app_hash"),
	}).Once().Return(&msm.ResponseOfferSnapshot{Result: msm.ResponseOfferSnapshot_REJECT}, nil)
...
func TestSyncer_SyncAny_msmError(t *testing.T) {
	syncer, connSnapshot := setupOfferSyncer(t)
...
func TestSyncer_offerSnapshot(t *testing.T) {
	...
	testcases := map[string]struct {
		...
		"accept":           {msm.ResponseOfferSnapshot_ACCEPT, nil, nil},
		"abort":            {msm.ResponseOfferSnapshot_ABORT, nil, errAbort},
		"reject":           {msm.ResponseOfferSnapshot_REJECT, nil, errRejectSnapshot},
		"reject_format":    {msm.ResponseOfferSnapshot_REJECT_FORMAT, nil, errRejectFormat},
		"reject_sender":    {msm.ResponseOfferSnapshot_REJECT_SENDER, nil, errRejectSender},
		"unknown":          {msm.ResponseOfferSnapshot_UNKNOWN, nil, unknownErr},
		"error":            {0, boom, boom},
	...
	for name, tc := range testcases {
		...
		t.Run(name, func(t *testing.T) {
			...
			connSnapshot.On("OfferSnapshotSync", msm.RequestOfferSnapshot{
				Snapshot: toMSM(s),
				AppHash:  []byte("app_hash"),
			}).Return(&msm.ResponseOfferSnapshot{Result: tc.result}, tc.err)
...
func TestSyncer_applyChunks_Results(t *testing.T) {
	...
	testcases := map[string]struct {
		result    msm.ResponseApplySnapshotChunk_Result
		err       error
		expectErr error
	}{
		"accept":           {msm.ResponseApplySnapshotChunk_ACCEPT, nil, nil},
		"abort":            {msm.ResponseApplySnapshotChunk_ABORT, nil, errAbort},
		"retry":            {msm.ResponseApplySnapshotChunk_RETRY, nil, nil},
		"retry_snapshot":   {msm.ResponseApplySnapshotChunk_RETRY_SNAPSHOT, nil, errRetrySnapshot},
		"reject_snapshot":  {msm.ResponseApplySnapshotChunk_REJECT_SNAPSHOT, nil, errRejectSnapshot},
		"unknown":          {msm.ResponseApplySnapshotChunk_UNKNOWN, nil, unknownErr},
		"error":            {0, boom, boom},
		"unknown non-zero": {9, nil, unknownErr},
	}
	for name, tc := range testcases {
		...
		t.Run(name, func(t *testing.T) {
			...
			connSnapshot.On("ApplySnapshotChunkSync", msm.RequestApplySnapshotChunk{
				Index: 0, Chunk: body,
			}).Once().Return(&msm.ResponseApplySnapshotChunk{Result: tc.result}, tc.err)
			if tc.result == msm.ResponseApplySnapshotChunk_RETRY {
				connSnapshot.On("ApplySnapshotChunkSync", msm.RequestApplySnapshotChunk{
					Index: 0, Chunk: body,
				}).Once().Return(&msm.ResponseApplySnapshotChunk{
					Result: msm.ResponseApplySnapshotChunk_ACCEPT}, nil)
			}
...
func TestSyncer_applyChunks_RefetchChunks(t *testing.T) {
	...
	testcases := map[string]struct {
		...
		"accept":          {msm.ResponseApplySnapshotChunk_ACCEPT},
		"abort":           {msm.ResponseApplySnapshotChunk_ABORT},
		"retry":           {msm.ResponseApplySnapshotChunk_RETRY},
		"retry_snapshot":  {msm.ResponseApplySnapshotChunk_RETRY_SNAPSHOT},
		"reject_snapshot": {msm.ResponseApplySnapshotChunk_REJECT_SNAPSHOT},
	}
	for name, tc := range testcases {
		tc := tc
		t.Run(name, func(t *testing.T) {
...
func TestSyncer_verifyApp(t *testing.T) {
	...
	testcases := map[string]struct {
		response  *msm.ResponseInfo
		err       error
		expectErr error
	}{
		"verified": {&msm.ResponseInfo{
			LastBlockHeight:  3,
			LastBlockAppHash: []byte("app_hash"),
			AppVersion:       9,
		}, nil, nil},
		"invalid height": {&msm.ResponseInfo{
			LastBlockHeight:  5,
			LastBlockAppHash: []byte("app_hash"),
			AppVersion:       9,
		}, nil, errVerifyFailed},
		"invalid hash": {&msm.ResponseInfo{
			LastBlockHeight:  3,
...
func toMSM(s *snapshot) *msm.Snapshot {
	return &msm.Snapshot{
		Height:   s.Height,
		Format:   s.Format,
		Chunks:   s.Chunks,
