...
func TestReactor_Receive_ChunkRequest(t *testing.T) {
	...
	for name, tc := range testcases {
		...
		t.Run(name, func(t *testing.T) {
			// Mock MSM connection to return local snapshots
			conn := &proxymocks.AppConnSnapshot{}
			conn.On("LoadSnapshotChunkSync", msm.RequestLoadSnapshotChunk{
				Height: tc.request.Height,
				Format: tc.request.Format,
				Chunk:  tc.request.Index,
			}).Return(&msm.ResponseLoadSnapshotChunk{Chunk: tc.chunk}, nil)
...
func TestReactor_Receive_SnapshotsRequest(t *testing.T) {
	...
	for name, tc := range testcases {
		...
		t.Run(name, func(t *testing.T) {
			// Mock MSM connection to return local snapshots
			conn := &proxymocks.AppConnSnapshot{}
			conn.On("ListSnapshotsSync", msm.RequestListSnapshots{}).Return(&msm.ResponseListSnapshots{
