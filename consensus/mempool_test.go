...
func TestMempoolRmBadTx(t *testing.T) {
	...
	resDeliver := app.DeliverTx(msm.RequestDeliverTx{Tx: txBytes})
	...
	go func() {
		...
		// CheckTx should not err, but the app should return a bad msm code
		// and the tx should get removed from the pool
		err := assertMempool(cs.txNotifier).CheckTx(txBytes, func(r *msm.Response) {
...
func (app *CounterApplication) Info(req msm.RequestInfo) msm.ResponseInfo {
	return msm.ResponseInfo{Data: fmt.Sprintf("txs:%v", app.txCount)}
}

func (app *CounterApplication) DeliverTx(req msm.RequestDeliverTx) msm.ResponseDeliverTx {
...
func (app *CounterApplication) CheckTx(req msm.RequestCheckTx) msm.ResponseCheckTx {
	txValue := txAsUint64(req.Tx)
	...
	app.mempoolTxCount++
	return msm.ResponseCheckTx{Code: code.CodeTypeOK}
}
...
func (app *CounterApplication) Commit() msm.ResponseCommit {
	...
	return msm.ResponseCommit{Data: hash}
}
