...
func TestSimulateValidatorsChange(t *testing.T) {
	...
	require.NoError(t, err)
	valPubKey1MSM, err := cryptoenc.PubKeyToProto(newValidatorPubKey1)
	require.NoError(t, err)
	newValidatorTx1 := kvstore.MakeValSetChangeTx(valPubKey1MSM, testMinPower)
	...
	require.NoError(t, err)
	updatePubKey1MSM, err := cryptoenc.PubKeyToProto(updateValidatorPubKey1)
	require.NoError(t, err)
	updateValidatorTx1 := kvstore.MakeValSetChangeTx(updatePubKey1MSM, 25)
	...
	require.NoError(t, err)
	newVal2MSM, err := cryptoenc.PubKeyToProto(newValidatorPubKey2)
	require.NoError(t, err)
	newValidatorTx2 := kvstore.MakeValSetChangeTx(newVal2MSM, testMinPower)
	...
	require.NoError(t, err)
	newVal3MSM, err := cryptoenc.PubKeyToProto(newValidatorPubKey3)
	require.NoError(t, err)
	newValidatorTx3 := kvstore.MakeValSetChangeTx(newVal3MSM, testMinPower)
	...
	removeValidatorTx2 := kvstore.MakeValSetChangeTx(newVal2MSM, 0)
	...
	height++
	incrementHeight(vss...)
	removeValidatorTx3 := kvstore.MakeValSetChangeTx(newVal3MSM, 0)
...
// Test mockProxyApp should not panic when app return MSMResponses with some empty ResponseDeliverTx
func TestMockProxyApp(t *testing.T) {
	...
	assert.NotPanics(t, func() {
		msmResWithEmptyDeliverTx := new(tmstate.MSMResponses)
		msmResWithEmptyDeliverTx.DeliverTxs = make([]*msm.ResponseDeliverTx, 0)
		msmResWithEmptyDeliverTx.DeliverTxs = append(msmResWithEmptyDeliverTx.DeliverTxs, &msm.ResponseDeliverTx{})

		// called when saveMSMResponses:
		bytes, err := proto.Marshal(msmResWithEmptyDeliverTx)
		require.NoError(t, err)
		loadedMsmRes := new(tmstate.MSMResponses)

		// this also happens sm.LoadMSMResponses
		err = proto.Unmarshal(bytes, loadedMsmRes)
		require.NoError(t, err)

		mock := newMockProxyApp([]byte("mock_hash"), loadedMsmRes)

		msmRes := new(tmstate.MSMResponses)
		msmRes.DeliverTxs = make([]*msm.ResponseDeliverTx, len(loadedMsmRes.DeliverTxs))
		...
		proxyCb := func(req *msm.Request, res *msm.Response) {
			if r, ok := res.Value.(*msm.Response_DeliverTx); ok {
				...
				msmRes.DeliverTxs[txIndex] = txRes
				txIndex++
			}
		}
		...
		someTx := []byte("tx")
		mock.DeliverTxAsync(msm.RequestDeliverTx{Tx: someTx})
	})
...
func testHandshakeReplay(t *testing.T, config *cfg.Config, nBlocks int, mode uint, testValidatorsChange bool) {
	...
	// run the chain through state.ApplyBlock to build up the augusteum state
	...
	if nBlocks > 0 {
		...
		// use a throwaway augusteum state
	...
	if expectError {
		...
		t.Fatalf("Error on msm handshake: %v", err)
	}

	// get the latest app hash from the app
	res, err := proxyApp.Query().InfoSync(msm.RequestInfo{Version: ""})
	if err != nil {
		t.Fatal(err)
	}
...
func buildAppStateFromChain(proxyApp proxy.AppConns, stateStore sm.Store,
	...
	if _, err := proxyApp.Consensus().InitChainSync(msm.RequestInitChain{
...
func buildTMStateFromChain(
	...
	nBlocks int,
	mode uint) sm.State {
	// run the whole chain against this client to build up the augusteum state
...
func TestHandshakePanicsIfAppReturnsWrongAppHash(t *testing.T) {
	// 1. Initialize augusteum and commit 3 blocks with the following app hashes:
	//		- 0x01
	//		- 0x02
	//		- 0x03
	...
	// 2. Augusteum must panic if app returns wrong hash for the first block
	//		- RANDOM HASH
	//		- 0x02
	//		- 0x03
	{
	...
	// 3. Augusteum must panic if app returns wrong hash for the last block
	//		- 0x01
	//		- 0x02
	//		- RANDOM HASH
	{
...
func (app *badApp) Commit() msm.ResponseCommit {
	app.height++
	if app.onlyLastHashIsWrong {
		if app.height == app.numBlocks {
			return msm.ResponseCommit{Data: tmrand.Bytes(8)}
		}
		return msm.ResponseCommit{Data: []byte{app.height}}
...
type initChainApp struct {
	msm.BaseApplication
	vals []msm.ValidatorUpdate
}

func (ica *initChainApp) InitChain(req msm.RequestInitChain) msm.ResponseInitChain {
