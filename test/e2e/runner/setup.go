...
import (
	...
	e2e "github.com/creatachain/augusteum/test/e2e/pkg"
...
func Setup(testnet *e2e.Testnet) error {
	...
	for _, node := range testnet.Nodes {
		...
		// Set up a dummy validator. Augusteum requires a file PV even when not used, so we
...
{{- range .Nodes }}
  {{ .Name }}:
    labels:
      e2e: true
    container_name: {{ .Name }}
    image: augusteum/e2e-node
{{- if eq .MSMProtocol "builtin" }}
...
func MakeGenesis(testnet *e2e.Testnet) (types.GenesisDoc, error) {
	...
	// The validator set will be sorted internally by Augusteum ranked by power,
...
// MakeConfig generates a Augusteum config for a node.
func MakeConfig(node *e2e.Node) (*config.Config, error) {
	...
	case e2e.ProtocolBuiltin:
		...
		cfg.MSM = ""
	default:
		return nil, fmt.Errorf("unexpected MSM protocol setting %q", node.MSMProtocol)
	}

	// Augusteum errors if it does not have a privval key set up, regardless of whether
...
// MakeAppConfig generates an MSM application config for a node.
