...
// Node represents a Augusteum node in a testnet.
type Node struct {
	Name             string
	Testnet          *Testnet
	Mode             Mode
...
func LoadTestnet(file string) (*Testnet, error) {
	...
	for _, name := range nodeNames {
		...
		if nodeManifest.MSMProtocol != "" {
			node.MSMProtocol = Protocol(nodeManifest.MSMProtocol)
		}
...
func (n Node) Validate(testnet Testnet) error {
	...
	switch n.MSMProtocol {
	case ProtocolBuiltin, ProtocolUNIX, ProtocolTCP, ProtocolGRPC:
	default:
		return fmt.Errorf("invalid MSM protocol setting %q", n.MSMProtocol)
	}
