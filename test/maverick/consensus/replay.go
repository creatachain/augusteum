...
func (h *Handshaker) Handshake(proxyApp proxy.AppConns) error {
	...
	// Handshake is done via MSM Info on the query conn.
	...
	h.logger.Info("MSM Handshake App Info",
		"height", blockHeight,
		"hash", appHash,
	...
	h.logger.Info("Completed MSM Handshake - Augusteum and App are synced",
...
func (h *Handshaker) ReplayBlocks(
	...
	case storeBlockHeight < stateBlockHeight:
		// the state should never be ahead of the store (this is under augusteum's control)
	...
	case storeBlockHeight > stateBlockHeight+1:
		// store should be at most one ahead of the state (this is under augusteum's control)
	...
	if storeBlockHeight == stateBlockHeight {
		// Augusteum ran Commit and saved the state.
		...
		case appBlockHeight == storeBlockHeight:
			...
			msmResponses, err := h.stateStore.LoadMSMResponses(storeBlockHeight)
			if err != nil {
				return nil, err
			}
			mockApp := newMockProxyApp(appHash, msmResponses)
...
Did you reset Augusteum without resetting your application's data?`,
