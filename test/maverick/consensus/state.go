...
func (cs *State) handleMsg(mi msgInfo) {
	...
	case *tmcon.VoteMessage:
		...
		// a typical peer.
		// https://github.com/creatachain/augusteum/issues/1281
		// }
...
func (cs *State) updateToState(state sm.State) {
	...
	case cs.LastCommit == nil:
		// NOTE: when Augusteum starts, it has no votes. reconstructLastCommit
...
func (cs *State) finalizeCommit(height int64) {
	...
	// running ApplyBlock during the MSM handshake when we restart.  If we
	...
	// Prune old heights, if requested by MSM app.
	if retainHeight > 0 {
...
func (cs *State) tryAddVote(vote *types.Vote, peerID p2p.ID) (bool, error) {
	...
	if err != nil {
		...
		if voteErr, ok := err.(*types.ErrVoteConflictingVotes); ok {
			...
			// 		(https://github.com/creatachain/augusteum/issues/3839).
...
func (cs *State) voteTime() time.Time {
	...
	// even if cs.LockedBlock != nil. See https://docs.augusteum.com/master/spec/.
	timeIota := time.Duration(cs.state.ConsensusParams.Block.TimeIotaMs) * time.Millisecond
	if cs.LockedBlock != nil {
		// See the BFT time spec https://docs.augusteum.com/master/spec/consensus/bft-time.html
