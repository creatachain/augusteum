...
###########################
# test using the msm-cli
###########################

echo "... testing query with msm-cli"

# we should be able to look up the key
RESPONSE=`msm-cli query \"$KEY\"`
...
#############################
# test using the /msm_query
#############################

echo "... testing query with /msm_query 2"

# we should be able to look up the key
RESPONSE=`curl -s "127.0.0.1:26657/msm_query?path=\"\"&data=$(toHex $KEY)&prove=false"`
...
# we should not be able to look up the value
RESPONSE=`curl -s "127.0.0.1:26657/msm_query?path=\"\"&data=$(toHex $VALUE)&prove=false"`
RESPONSE=`echo $RESPONSE | jq .result.response.log`
set +e
A=`echo $RESPONSE | grep 'exists'`
if [[ $? == 0 ]]; then
