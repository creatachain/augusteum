...
type Evidence interface {
	MSM() []msm.Evidence  // forms individual evidence to be sent to the application
...
// MSM returns the application relevant representation of the evidence
func (dve *DuplicateVoteEvidence) MSM() []msm.Evidence {
	return []msm.Evidence{{
...
// augusteum/docs/architecture/adr-047-handling-evidence-from-light-client.md
...
// MSM forms an array of msm evidence for each byzantine validator
func (l *LightClientAttackEvidence) MSM() []msm.Evidence {
	msmEv := make([]msm.Evidence, len(l.ByzantineValidators))
	for idx, val := range l.ByzantineValidators {
		msmEv[idx] = msm.Evidence{
...
func init() {
	tmjson.RegisterType(&DuplicateVoteEvidence{}, "augusteum/DuplicateVoteEvidence")
	tmjson.RegisterType(&LightClientAttackEvidence{}, "augusteum/LightClientAttackEvidence")
}
