...
func DefaultValidatorParams() tmproto.ValidatorParams {
	return tmproto.ValidatorParams{
		PubKeyTypes: []string{MSMPubKeyTypeEd25519},
	}
}
...
func ValidateConsensusParams(params tmproto.ConsensusParams) error {
	...
	// Check if keyType is a known MSMPubKeyType
	for i := 0; i < len(params.Validator.PubKeyTypes); i++ {
		keyType := params.Validator.PubKeyTypes[i]
		if _, ok := MSMPubKeyTypesToNames[keyType]; !ok {
...
func UpdateConsensusParams(params tmproto.ConsensusParams, params2 *msm.ConsensusParams) tmproto.ConsensusParams {
