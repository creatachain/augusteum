...
import (
	...
	auto "github.com/creatachain/augusteum/libs/autofile"
...
// CheckTx msm message before the transaction is added to the pool. The
...
type CListMempool struct {
	...
	// serial (ie. by msm responses which are called in serial).
...
//
// Safe for concurrent use by multiple goroutines.
func (mem *CListMempool) CheckTx(tx types.Tx, cb func(*msm.Response), txInfo TxInfo) error {
	mem.updateMtx.RLock()
	...
	reqRes := mem.proxyAppConn.CheckTxAsync(msm.RequestCheckTx{Tx: tx})
	reqRes.SetCallback(mem.reqResCb(tx, txInfo.SenderID, txInfo.SenderP2PID, cb))

	return nil
}

// Global callback that will be called after every MSM response.
// Having a single global callback avoids needing to set a callback for each request.
// However, processing the checkTx response requires the peerID (so we can track which txs we heard from who),
// and peerID is not included in the MSM request, so we have to set request-specific callbacks that
// include this information. If we're not in the midst of a recheck, this function will just return,
// so the request specific callback can do the work.
//
// When rechecking, we don't need the peerID, so the recheck callback happens
// here.
func (mem *CListMempool) globalCb(req *msm.Request, res *msm.Response) {
...
// NOTE: alternatively, we could include this information in the MSM request itself.
//
...
func (mem *CListMempool) reqResCb(
	...
	peerID uint16,
	peerP2PID p2p.ID,
	externalCb func(*msm.Response),
) func(res *msm.Response) {
	return func(res *msm.Response) {
		if mem.recheckCursor != nil {
...
func (mem *CListMempool) resCbFirstTime(
	...
	case *msm.Response_CheckTx:
		var postCheckErr error
		if mem.postCheck != nil {
			postCheckErr = mem.postCheck(tx, r.CheckTx)
		}
		if (r.CheckTx.Code == msm.CodeTypeOK) && postCheckErr == nil {
...
func (mem *CListMempool) resCbRecheck(req *msm.Request, res *msm.Response) {
	switch r := res.Value.(type) {
	case *msm.Response_CheckTx:
...
func (mem *CListMempool) Update(
	...
	for i, tx := range txs {
		if deliverTxResponses[i].Code == msm.CodeTypeOK {
		...
		// Mempool after:
		//   100
		// https://github.com/creatachain/augusteum/issues/3322.
		if e, ok := mem.txsMap.Load(TxKey(tx)); ok {
...
func (mem *CListMempool) recheckTxs() {
	...
	for e := mem.txs.Front(); e != nil; e = e.Next() {
		memTx := e.Value.(*mempoolTx)
		mem.proxyAppConn.CheckTxAsync(msm.RequestCheckTx{
			Tx:   memTx.tx,
			Type: msm.CheckTxType_Recheck,
		})
	}
