...
func newMempoolWithAppAndConfig(cc proxy.ClientCreator, config *cfg.Config) (*CListMempool, cleanupFunc) {
	...
	appConnMem.SetLogger(log.TestingLogger().With("module", "msm-client", "connection", "mempool"))
...
func TestReapMaxBytesMaxGas(t *testing.T) {
	...
	require.Equal(t, app.CheckTx(msm.RequestCheckTx{Tx: tx0.tx}).GasWanted, int64(1), "KVStore had a gas value neq to 1")
...
func TestMempoolFilters(t *testing.T) {
	...
	nopPostFilter := func(tx types.Tx, res *msm.ResponseCheckTx) error { return nil }
	...
	for tcIndex, tt := range tests {
		err := mempool.Update(1, emptyTxArr, msmResponses(len(emptyTxArr), msm.CodeTypeOK), tt.preFilter, tt.postFilter)
...
func TestMempoolUpdate(t *testing.T) {
	...
	{
		err := mempool.Update(1, []types.Tx{[]byte{0x01}}, msmResponses(1, msm.CodeTypeOK), nil, nil)
		require.NoError(t, err)
	...
	// 2. Removes valid txs from the mempool
	{
		...
		require.NoError(t, err)
		err = mempool.Update(1, []types.Tx{[]byte{0x02}}, msmResponses(1, msm.CodeTypeOK), nil, nil)
		require.NoError(t, err)
	...
	// 3. Removes invalid transactions from the cache and the mempool (if present)
	{
		...
		require.NoError(t, err)
		err = mempool.Update(1, []types.Tx{[]byte{0x03}}, msmResponses(1, 1), nil, nil)
		require.NoError(t, err)
		assert.Zero(t, mempool.Size())
...
func TestMempool_KeepInvalidTxsInCache(t *testing.T) {
	...
	// 1. An invalid transaction must remain in the cache after Update
	{
		...
		// simulate new block
		_ = app.DeliverTx(msm.RequestDeliverTx{Tx: a})
		_ = app.DeliverTx(msm.RequestDeliverTx{Tx: b})
		err = mempool.Update(1, []types.Tx{a, b},
			[]*msm.ResponseDeliverTx{{Code: msm.CodeTypeOK}, {Code: 2}}, nil, nil)
		require.NoError(t, err)
...
func TestTxsAvailable(t *testing.T) {
	...
	if err := mempool.Update(1, committedTxs, msmResponses(len(committedTxs), msm.CodeTypeOK), nil, nil); err != nil {
		t.Error(err)
	}
	ensureFire(t, mempool.TxsAvailable(), timeoutMS)
	ensureNoFire(t, mempool.TxsAvailable(), timeoutMS)
	...
	if err := mempool.Update(2, committedTxs, msmResponses(len(committedTxs), msm.CodeTypeOK), nil, nil); err != nil {
		t.Error(err)
	}
	ensureNoFire(t, mempool.TxsAvailable(), timeoutMS)
...
func TestSerialReap(t *testing.T) {
	app := counter.NewApplication(true)
	app.SetOption(msm.RequestSetOption{Key: "serial", Value: "on"})
	cc := proxy.NewLocalClientCreator(app)

	mempool, cleanup := newMempoolWithApp(cc)
	defer cleanup()

	appConnCon, _ := cc.NewMSMClient()
	appConnCon.SetLogger(log.TestingLogger().With("module", "msm-client", "connection", "consensus"))
	...
	updateRange := func(start, end int) {
		...
		if err := mempool.Update(0, txs, msmResponses(len(txs), msm.CodeTypeOK), nil, nil); err != nil {
			t.Error(err)
		}
	}
	...
	commitRange := func(start, end int) {
		...
		for i := start; i < end; i++ {
			txBytes := make([]byte, 8)
			binary.BigEndian.PutUint64(txBytes, uint64(i))
			res, err := appConnCon.DeliverTxSync(msm.RequestDeliverTx{Tx: txBytes})
			if err != nil {
...
func TestMempoolTxsBytes(t *testing.T) {
	...
	err = mempool.Update(1, []types.Tx{}, msmResponses(0, msm.CodeTypeOK), nil, nil)
	require.NoError(t, err)
	assert.EqualValues(t, 0, mempool.TxsBytes())
...
// https://github.com/creatachain/augusteum/issues/3509
...
func newRemoteApp(
	...
	// Start server
	server = msmserver.NewSocketServer(addr, app)
	server.SetLogger(log.TestingLogger().With("module", "msm-server"))
...
func msmResponses(n int, code uint32) []*msm.ResponseDeliverTx {
	responses := make([]*msm.ResponseDeliverTx, 0, n)
	for i := 0; i < n; i++ {
		responses = append(responses, &msm.ResponseDeliverTx{Code: code})
	}
	return responses
}
