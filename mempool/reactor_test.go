...
import (
	...
	"github.com/creatachain/augusteum/p2p/mock"
	memproto "github.com/creatachain/augusteum/proto/augusteum/mempool"
...
// regression test for https://github.com/creatachain/augusteum/issues/5408
func TestReactorConcurrency(t *testing.T) {
	...
	for i := 0; i < 1000; i++ {
		...
		go func() {
			...
			deliverTxResponses := make([]*msm.ResponseDeliverTx, len(txs))
			for i := range txs {
				deliverTxResponses[i] = &msm.ResponseDeliverTx{Code: 0}
			}
		...
		go func() {
			...
			err := reactors[1].mempool.Update(1, []types.Tx{}, make([]*msm.ResponseDeliverTx, 0), nil, nil)
			assert.NoError(t, err)
		}()
