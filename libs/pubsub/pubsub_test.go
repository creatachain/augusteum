...
func TestDifferentClients(t *testing.T) {
	...
	subscription2, err := s.Subscribe(
		ctx,
		"client-2",
		query.MustParse("tm.events.type='NewBlock' AND msm.account.name='Igor'"),
	)
	require.NoError(t, err)
	err = s.PublishWithEvents(
		ctx,
		"Ultimo",
		map[string][]string{"tm.events.type": {"NewBlock"}, "msm.account.name": {"Igor"}},
	)
	require.NoError(t, err)
	...
	subscription3, err := s.Subscribe(
		ctx,
		"client-3",
		query.MustParse("tm.events.type='NewRoundStep' AND msm.account.name='Igor' AND msm.invoice.number = 10"),
	)
	require.NoError(t, err)
...
func benchmarkNClients(n int, b *testing.B) {
	...
	for i := 0; i < n; i++ {
		subscription, err := s.Subscribe(
			ctx,
			clientID,
			query.MustParse(fmt.Sprintf("msm.Account.Owner = 'Ivan' AND msm.Invoices.Number = %d", i)),
		)
		if err != nil {
			b.Fatal(err)
		}
	...
	for i := 0; i < b.N; i++ {
		err = s.PublishWithEvents(
			ctx,
			"Gamora",
			map[string][]string{"msm.Account.Owner": {"Ivan"}, "msm.Invoices.Number": {string(rune(i))}},
		)
		require.NoError(b, err)
	}
}
...
func benchmarkNClientsOneQuery(n int, b *testing.B) {
	...
	ctx := context.Background()
	q := query.MustParse("msm.Account.Owner = 'Ivan' AND msm.Invoices.Number = 1")
	for i := 0; i < n; i++ {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		err = s.PublishWithEvents(ctx, "Gamora", map[string][]string{"msm.Account.Owner": {"Ivan"},
			"msm.Invoices.Number": {"1"}})
		require.NoError(b, err)
	}
}
